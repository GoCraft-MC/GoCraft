...
type CommandContext struct {
  ...
  // SyncCommandPermissions resends the command tree to a player after their
  // operator status changes, so the Java client immediately sees the updated
  // set of available commands without requiring a reconnect.
  SyncCommandPermissions func(*player.Player)
...
// SetCommandPermissionSync installs the bridge that resends the command tree
// to a Java player after their operator status changes.
func (d *Dispatcher) SetCommandPermissionSync(sync func(*player.Player)) {
  d.mu.Lock()
  d.syncCommandPermissions = sync
  d.mu.Unlock()
}
...
func (d *Dispatcher) Dispatch(input string, ctx CommandContext) {
  ...
  syncCommandPermissions := d.syncCommandPermissions
  d.mu.RUnlock()
  ...
  ctx.SyncCommandPermissions = syncCommandPermissions
