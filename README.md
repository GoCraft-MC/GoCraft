...
| Minecraft: Bedrock Edition 1.26.40 / protocol 2168 | Beta native adapter with canonical-world and Java/Bedrock cross-play support |
...
- **Canonical world layer** (`core/world`):
  ...
  - Deterministic seeded Overworld, Nether, and End terrain with dimension biomes, caves, ores, vegetation, portals, End outer islands, and obsidian spikes
...
| `mob_spawning.*` | Natural-mob caps per complete 17x17 chunk simulation area; smaller areas scale down and `0` disables a category |
...
| `world_seed` | Signed 64-bit seed for deterministic Overworld, Nether, and End terrain |
...
Disk mode creates `world_dir/region`, `world_dir/DIM-1/region`, and `world_dir/DIM1/region` for the Overworld, Nether, and End. It keeps bounded hot-chunk caches in RAM, commits generated and modified chunks every 30 seconds, and flushes again on clean shutdown. Memory mode performs no disk reads or writes, so its world changes are lost on restart. Go may retain freed heap pages for reuse, so panel-reported RAM does not always fall immediately even when cached chunks are evicted.

Changing `world_seed` or generation behavior affects newly generated chunks only. Use a new `world_dir` when changing seeds to avoid terrain seams. The built-in generator creates Overworld continents and climate terrain; the Nether's five vanilla biomes, lava sea, caves, ores, forests and basalt features; and the End's central island, arrival/exit structures, obsidian spikes, inner void, outer islands, End biomes, and chorus plants. The native Go pipeline follows Pumpkin's generation stages but is not block-for-block seed-compatible with Mojang or Pumpkin.

The built-in generator currently creates villages and the essential dimension landmarks, but not every vanilla structure: mineshafts, ancient cities, trial chambers, trail ruins, outposts, mansions, monuments, shipwrecks, ocean ruins, ruined portals, desert wells, fossils, bastions, fortresses, and End cities are not generated yet. Importing pre-generated Anvil dimension folders remains the way to use those structures today.
...
Both clients now observe the same `core/world.World`, with each adapter converting canonical state into its own protocol independently. Bedrock receives Pumpkin/BDS 1.26.40's complete 1,976-entry item registry (including data-driven components) and 1,875-entry grouped Creative catalogue. The implementation is still beta: some specialised container transactions, exact biome palettes, and complete parity for every vanilla UI, sound, particle, and specialised entity behaviour remain future work.
