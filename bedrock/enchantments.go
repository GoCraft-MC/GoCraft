...
var bedrockEnchantmentIDs = map[string]int16{
	...
	"minecraft:mending": 26, "minecraft:binding_curse": 27,
	"minecraft:vanishing_curse": 28, "minecraft:impaling": 29,
...
func bedrockEnchantments(stack player.ItemStack) []map[string]any {
	values := make([]map[string]any, 0, len(stack.EnchantmentLevels()))
	for _, enchantment := range stack.EnchantmentLevels() {
		id, ok := bedrockEnchantmentIDs[enchantment.ID]
		if !ok {
			continue
		}
		values = append(values, map[string]any{
			"id": id, "lvl": int16(enchantment.Level),
		})
	}
	return values
}
