...
func (b *Bus) RegisterMetrics(registry prometheus.Registerer) {
	...
	m := &busMetrics{
		...
		cold:     histogram("cold_start_seconds", "Duration of first dispatches per plugin event subscription in seconds."),
...
func (b *Bus) recordDispatch(sub *subscriber, event string, took time.Duration, failed, cold bool) {
	...
	if m := b.metrics; m != nil {
		...
		if cold {
			m.cold.WithLabelValues(sub.id, event).Observe(took.Seconds())
		}
	}
}
